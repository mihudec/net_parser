...
class TestBaseConfigParser(BaseNetParserTest):
    ...
    SAMPLE_CONFIG = BaseConfigParser(
        config=BaseNetParserTest.RESOURCES_DIR.joinpath('ios').joinpath('data').joinpath('sample_config_01.txt'))
    SAMPLE_CONFIG.parse()
    ...
    def test_get_parent(self):
        ...
        with self.subTest(msg="Router BGP - Get All Parents from neighbor activate"):
            neighbor_activate_line = [x for x in self.SAMPLE_CONFIG.lines if x.text == "  neighbor 192.0.2.2 activate"][
                0]
    ...
    def test_get_line(self):
        with self.subTest(msg="Router BGP - Get line from neighbor activate"):
            neighbor_activate_line = [x for x in self.SAMPLE_CONFIG.lines if x.text == "  neighbor 192.0.2.2 activate"][
                0]
            self.assertEqual(neighbor_activate_line.get_line,
                             ['router bgp 65535', ' address-family vpnv4', '  neighbor 192.0.2.2 activate'])
