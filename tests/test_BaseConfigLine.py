...
class TestBaseConfigParser(BaseNetParserTest):
    ...
    def test_get_children(self):
        with self.subTest(msg="Interface Vlan1 - Get Children from Interface"):
            parent_line = [x for x in self.SAMPLE_CONFIG if x.cdp_text == "interface Vlan1"][0]
        ...
        with self.subTest(msg="Router BGP - Get only direct children"):
            parent_line = [x for x in self.SAMPLE_CONFIG if x.cdp_text == "router bgp 65535"][0]
    ...
    def test_get_parent(self):
        ...
        with self.subTest(msg="Interface Vlan1 - Get Parent from description"):
            description_line = [x for x in self.SAMPLE_CONFIG.lines if x.cdp_text == " description Test Interface Vlan1"][0]
            interface_line = [x for x in self.SAMPLE_CONFIG.lines if x.cdp_text == "interface Vlan1"][0]
            self.assertEqual(description_line.parent, interface_line)
        ...
        with self.subTest(msg="Router BGP - Get All Parents from neighbor activate"):
            neighbor_activate_line = [x for x in self.SAMPLE_CONFIG.lines if x.cdp_text == "  neighbor 192.0.2.2 activate"][0]
    ...
    def test_get_section_by_parents(self):
        with self.subTest(msg="Router BGP - Get line from neighbor activate"):
            neighbor_activate_line = [x for x in self.SAMPLE_CONFIG.lines if x.cdp_text == "  neighbor 192.0.2.2 activate"][0]
            parent_1 = [x for x in self.SAMPLE_CONFIG if x.cdp_text == "router bgp 65535"][0]
            parent_2 = [x for x in self.SAMPLE_CONFIG if x.cdp_text == " address-family vpnv4" and x.parent == parent_1][0]
            parents = [parent_1, parent_2]
            print(parents)
            print(self.SAMPLE_CONFIG.get_section_by_parents(parents=[x.cdp_text for x in parents]))
